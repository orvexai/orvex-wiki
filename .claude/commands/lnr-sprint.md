...
2. Read `.cache/linear/work-status.yaml` from the repo-local cache (gitignored). Render the milestone -> issues overview directly from `work-status.yaml`'s `milestones:` and `issues:` maps (group issue entries by their `milestone:` key).

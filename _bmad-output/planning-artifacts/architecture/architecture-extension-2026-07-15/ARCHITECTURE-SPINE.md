---
name: Orvex Studio Extension — Cross-AI Delivery
type: architecture-spine
purpose: build-substrate
altitude: feature
paradigm: ports-and-adapters delivery client (MV3)
scope: Cross-AI delivery browser-extension component (the AD-5a client compose port); realizes F1 delivery for ENG-2690 and binds build stories ENG-2711..2730.
status: final
created: 2026-07-15
updated: 2026-07-15
binds: [ENG-2690, "ENG-2711..2730", AD-5a, AD-6, AD-8, AD-11, I-1, I-4]
sources: [iiCcKhGptV, g9vWbSYplh, QpVnEF7aEU, 5t8uxVnnA4, V6hlDjecfh, IgOjzk034v, 6aMAzsYeQb, 8sYi523i4t]
companions: [QpVnEF7aEU]
---
...
```mermaid
flowchart LR
  A["resolve<br/>compose node"] --> B{"verify-writable?"}
  B -- no --> X1["NOTWRITABLE<br/>abort before visible write"]
  B -- yes --> C["insert<br/>native-setter + InputEvent"]
  C --> D["read-back<br/>~250ms budget"]
  D -- confirmed --> S["success<br/>visible in composer"]
  D -- not confirmed --> X2["VERIFY<br/>fail loud"]
  A -- not found --> X3["NOTFOUND<br/>fail loud"]
  A -- multiple candidates --> X4["AMBIGUOUS<br/>fail loud"]
  X1 --> F["copy paste fallback<br/>plus team alert"]
  X2 --> F
  X3 --> F
  X4 --> F
```

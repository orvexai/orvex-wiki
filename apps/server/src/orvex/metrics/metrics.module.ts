 ...
 * mounts the SHARED `OrvexMetricsService` (engine-resident since ENG-3149:
 * inlined from the retired AGPL shared metrics package, AD-8/AD-9) into
 * this engine, and the `/metrics` HTTP handler that exposes it.
 *
 ...
 * po-ruling 10 forbids. `OrvexMetricsService` is the shared, already-tested
 * engine-resident registry (`./metrics-service`, ENG-1610/ENG-3149) — it is
 * constructed here, not wrapped or re-implemented (§3.1 deletion test: this
 * module's value is the fail-closed authz + route-exclusion the raw service
 * does not provide).
 */
@Global()
@Module({

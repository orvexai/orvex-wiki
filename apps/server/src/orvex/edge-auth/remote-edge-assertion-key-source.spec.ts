 ...
 * ES256 keypair generated by Node's Web Crypto (no hand-authored key, and no
 * JWT/JOSE library — AD-9, ENG-3495). The behaviours under test are the ones
...
async function buildJwks(): Promise<{ jwks: EdgeJwks }> {
  ...
  // Strip the export-only fields WebCrypto adds so the fixture is shaped like
  // a real identity JWKS entry rather than a WebCrypto export blob.
  delete jwk.key_ops;
  delete jwk.ext;
  const jwks: EdgeJwks = {

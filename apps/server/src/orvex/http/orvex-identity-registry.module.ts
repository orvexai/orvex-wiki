...
export function composeIdentityRegistryClient(
  ...
  return new HttpIdentityRegistryClient({
    ...
    // ENG-3350/ENG-3313 — the shared engine seam credential for identity's
    // origin-locked registry WRITES (POST /internal/registry/reserve and
    // POST /internal/registry/move). Read here (the ONE composition point)
    // rather than at either call site; null when unset, which the client
    // turns into a typed refusal, never an unauthenticated call.
